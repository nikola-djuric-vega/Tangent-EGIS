...
import { PageTypeNames, SectorPage as SectorPageType } from '../../types/CMS'
...
export const SectorPage = ({ data }: SectorPageProps) => {
  const subSectorLinks = data?.descendants?.items?.filter((item: any) => {
    if (item.__typename === PageTypeNames.SUB_SECTOR_PAGE) {
      return item
    }
  })
